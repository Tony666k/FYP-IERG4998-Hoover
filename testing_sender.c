...
// Define a constant message with 4 '0's
#define ZERO_MESSAGE "0000"
...
// Thread function for sending binary data to port 10002 (sending 0000)
void* send_to_10002(void *arg) {
    const char *server_ip = "192.168.229.135"; // Our server IP addr
    const char *client_ip = "192.168.229.134"; // Our client IP addr
    int client_port = 10002;

    // Send the fixed "0000" message to port 10002 once
...
int main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        // Start sending data to port 10002 (sending "0000")
