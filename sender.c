...
void send_udp_message(const char *source_ip,int source_port,const char *targ>
int sockfd;
...
sendto(sockfd,(const char *)message,strlen(message),0,(const struct sockaddr>
printf("Message sent from %s:%d to %s:%d\n",source_ip,source_port,target_ip,>
close(sockfd);
}
